...
func TestPrometheusTimeoutHTTP(t *testing.T) {
	moduleUnknownCounter := prometheus.NewCounter(prometheus.CounterOpts{
		Name: "blackbox_exporter_unknown_module_total",
		Help: "Total probes that used an unknown module.",
	})
	...
	handler := http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		params := r.URL.Query()
		Handler(w, r, c, log.NewNopLogger(), &ResultHistory{}, 0.5, params, moduleUnknownCounter, level.AllowNone())
	})
